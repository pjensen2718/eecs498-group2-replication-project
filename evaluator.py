...
def get_prompts(prompts_path) -> list[str]:
    ...
    dataframe = pd.read_csv(prompts_path, delimiter='|')
...
def intro(models: tuple[str], prompts_path: str, all_official: bool) -> list[str]:
    ...
    if prompts_path:
        return get_prompts(prompts_path)
...
def grade_completion_with_gpt(story_completion: str) -> tuple[str, str]:
    ...
    client = OpenAI() # TODO: investigate if defining this every time impacts runtime
...
@click.option("-p", "--prompts_path", type=str, default="prompts.csv", help="Specify path to prompts file.")
...
def main(models, prompts_path, all_official, num_completions, verbose):
    ...
    # currently, use this as "python3 evaluator.py -m roneneldan/TinyStories-1M --prompts_path prompts.csv"
